...
var dupSub = function(req, res, callback){
    ...
    if(database.db){
        database.db.connect(function(err){
           var sql = "SELECT subscription FROM Subscribers WHERE JSON_EXTRACT(subscription, '$.endpoint') = ?" 
           var value = [
               endpoint
           ];
    ...
    console.log('subscription 모듈 안에 있는 addDB 호출됨.');
        ...
        if(database.db){
            database.db.connect(function(err){
                ...
                console.log('추가 요청 key: ',req.body['key']);
                
                var cafe_sql = req.body['addDBSql'];
                var cafe_value = req.body['addDBValue'].split('&');
                
                var cafe_values= [cafe_value];
                
                database.db.query(cafe_sql, [cafe_values], function(err, result){
                   if(err) throw err;
                    console.log('SubscribersDB insert 완료!');
                    
                    //관리자에게 새로운 등록을 알림.
                    var sql2 = "SELECT * FROM Admin WHERE adminKey=?";
                    var value2 = [
                        req.body['key']
                    ];
    ...
    if(database.db){
        database.db.connect(function(err){
           console.log('delDB 함수 안에서 DB 연결됨.');
            var cafe_sql = req.body['delDBSql'];
            var value =[
                delsub.endpoint
            ];
            database.db.query(cafe_sql, value, function(err, result){
               if(err) throw err;
                ...
                console.log('SubscribersDB delete 완료!');
    ...
    if(database.db){
        database.db.connect(function(err){
           console.log('findClient 함수 안에서 DB 연결됨.');
            ...
            }else if(req.body['tag']==='customize'){
                var sql = "SELECT * FROM Subscribers WHERE idx=?";
                var value = [
                    req.body['idx']
                ];
                database.db.query(sql, value, function(err, result){
                    if(err) throw err;
                    console.log('CUSTOMIZE TAG: 해당 Obj 찾음.');
...
var push2Client = function(req, result, res, tag){
    ...
    var vapidPublicKey='BBrasy-diut9lm_V7t9FGxUzbpkwKz87W7g__TheGvHFpGnd0hGn99oVgnthtm4erKXXNx6Rok1ezbhhTlboBtw';
    var vapidPrivateKey='YOUR OWN PRIVATE KEY';
    ...
    if(tag == 0){
        ...
        console.log('customize: ' + req.body['idx'] +', '+req.body['title']+', '+req.body['content']+', '+req.body['dest']);
        payloadJSON.tag = "customize";
        payloadJSON.title = req.body['title'];
        payloadJSON.content = req.body['content'];
        payloadJSON.dest=req.body['dest'];
    }else if(tag == 2){
        payloadJSON.tag = "manage";
    }
...
var reception = function(req, res){
    ...
    if(database.db){
        database.db.connect(function(err){
           var sql = "UPDATE Subscribers SET reception = '수신 확인',time=?,subscription=? WHERE JSON_EXTRACT(subscription, '$.endpoint') = ?"; 
            var value = [
                date,
            ...
            database.db.query(sql, value, function(err, result){
               ...
               console.log('reception 변경 완료!');
                ...
                var sql2 = "SELECT * FROM Admin WHERE subscription IS NOT NULL";
                var value2 = [
