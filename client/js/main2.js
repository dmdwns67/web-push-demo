...
if ('serviceWorker' in navigator && 'PushManager' in window) {
    ...
    navigator.serviceWorker.register('/client/sw-client-samsung.js')
    .then(function(swReg) {
...
function subscribeUser() {
    ...
    .then(function(subscription) {   
        ...
        $.ajax({
            type: 'POST',
            url: 'https://chungullim.com:3000/process/addSub',
            async: true, //비동기식
            ...
            data: JSON.stringify({
                ...
                "key": 'chungullim',
                "addDBSql": "INSERT INTO Subscribers(nickname, subscription, time, reception, subKey) VALUES ?",
                "addDBValue": "chungullim구독자&"+ JSON.stringify(subscription) +"&"+ dateTime + "&수신 대기&chungullim"
            }),
            dataType:'json',
            processData: true,
         ...
         alert('웹브라우저 창을 닫으셔도 알림을 받으실 수 있습니다. 절전모드 시에는 서비스가 원활하지 않을 수 있습니다.');
...
function unsubscribeUser() {
      swRegistration.pushManager.getSubscription()
          ...
          $.ajax({
                type: 'POST',
                url: 'https://chungullim.com:3000/process/delSub',
                contentType: 'application/json',
                async: true,
                data: JSON.stringify({
                    "subscriptionJson": JSON.stringify(delSub),
                    "payload": 'chungullim구독자가 삭제되었습니다.',
                    "key": 'chungullim',
                    "delDBSql":"DELETE FROM Subscribers WHERE JSON_EXTRACT(subscription, '$.endpoint') = ?",
                }),
                dataType:'json',
                processData: true,
                success: function(msg){
